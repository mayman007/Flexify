...
class LockscreenDetailsView extends StatefulWidget {
  final String lockscreenUrl;
  final String lockscreenThumbnailUrl;
  final String lockscreenName;

  const LockscreenDetailsView({
    super.key,
    required this.lockscreenUrl,
    required this.lockscreenThumbnailUrl,
    required this.lockscreenName,
  });

  @override
  State<LockscreenDetailsView> createState() => _LockscreenDetailsViewState();
}

class _LockscreenDetailsViewState extends State<LockscreenDetailsView> {
  ...
  checkIfFaved() async {
    ...
    for (var row in table) {
      if (widget.lockscreenUrl == row['lockscreenurl']) {
        setState(() {
          isFaved = true;
        });
        break;
  ...
  insertOrDeleteFaved() async {
    if (isFaved) {
      await sqlDb.deleteData(
          "DELETE FROM 'lockscreenfavs' WHERE lockscreenurl = '${widget.lockscreenUrl}'");
      setState(() {
        isFaved = false;
      });
      showToast(
      ...
      await sqlDb.insertData(
          "INSERT INTO 'lockscreenfavs' ('lockscreenurl', 'lockscreenname' )VALUES ('${widget.lockscreenUrl}', '${widget.lockscreenName}' )");
      setState(() {
        isFaved = true;
      });
      showToast(
  ...
  applyLockscreen() async {
    ...
    String fullPath = "${tempDir.path}/${widget.lockscreenName}.klwp";

    // Download file
    Response response = await Dio().get(
      widget.lockscreenUrl,
      options: Options(
